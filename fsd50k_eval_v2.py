...
import tqdm
import numpy as np
from src.utilities import metrics_helper
...
if __name__ == '__main__':
    ...
    val_set = RawWaveformDataset("/media/user/nvme/datasets/fsd50k/fsd50k_8000/meta/eval.csv",
                                 "/media/user/nvme/datasets/fsd50k/fsd50k_8000/meta/lbl_map.json",
                                 hparams.cfg['audio_config'], mode='multilabel',
                                 transform=val_tfs, is_val=True
                                 )

    with open("/media/user/nvme/datasets/fsd50k/fsd50k_8000/meta/lbl_map.json", "r") as fd:
    ...
    val_gts = torch.cat(val_gts, 0).numpy()

    stats = metrics_helper.calculate_stats(val_preds, val_gts)
    mAP = np.mean([stat['AP'] for stat in stats])
    mAUC = np.mean([stat['auc'] for stat in stats])
    dp = metrics_helper.d_prime(mAUC)
    print("mAP: {} | mAUC: {} | dprime: {}".format(mAP, mAUC, dp))
    fname = ckpt_path.split("/")[-3]

    ckpt_ext = "/".join(ckpt_path.split("/")[-3:])
    with open(res_file, "w") as fd:
        fd.writelines("model,mAP,mAUC,dprime,ckpt_ext\n")
        fd.writelines("{},{},{},{},{}\n".format(fname, macro_mAP, mAUC, dp, ckpt_ext))
