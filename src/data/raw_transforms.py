...
class NonOverlappingRandomCrop:
    def __init__(self, size, sample_rate, min_separation_ms=250):
        ...
        print(self.min_separation)

    def __call__(self, signal):
        mid = int(signal.shape[1] / 2)
        idx1 = random.randint(0, mid - self.size)
        idx2 = random.randint(idx1+self.size+self.min_separation, signal.shape[1] - self.size)
        # idxs = [idx1, idx2]
        # idx1, idx2 = np.random.permutation([idx1, idx2])
        choice = np.random.binomial(1, 0.5)
        if choice == 0:
            return signal[:, idx1:idx1+self.size], signal[:, idx2:idx2+self.size]
        else:
            return signal[:, idx2:idx2+self.size], signal[:, idx1:idx1+self.size]
...
class TimeMasking:
    ...
    def __call__(self, x):
        ...
        time1 = time.time()
        # print("[time masking] min: {} | max: {}".format(x.min(), x.max()))
        # print("[time masking] time:", time1-time0)
        return x
...
class ClipValue:
    ...
    def __call__(self, x):
        ...
        # print("[clip] min: {} | max: {}".format(x.min(), x.max()))
        return x
...
class RandomReverb:
    ...
    def __call__(self, x):
        ...
        t1 = time.time()
        # print("[reverb] min: {} | max: {}".format(x.min(), x.max()))
        # print("[reverb] time:", t1 - t0)
        return x
...
class BackgroundNoiseGenerator:
    ...
    def __call__(self):
        ...
        # print("[in noisegen] audio.shape", audio.shape)
        return audio


class BackgroundNoiseLMDB:
    def __init__(self, dump_path,
                 sr=16000,
                 min_duration=5, waveform_parser=None):
        assert os.path.exists(noise_path)
        self.files = glob.glob(os.path.join(noise_path, "*"))
        self.in_memory = in_memory
        self.sr = sr
        self.min_duration = min_duration
        self.waveform_parser = waveform_parser
...
class RandomGain:
    ...
    def __call__(self, x):
        ...
        spec = self.gain(spec)
        t1 = time.time()
        # print("[random gain] min: {} | max: {}".format(spec.min(), spec.max()))
        # print("[gain] time: ", t1 - t0)
        return spec[0]
...
class PeakNormalization:
    ...
    def __call__(self, x):
        ...
        t1 = time.time()
        # print("[peak norm] time: ", t1-t0)
        # print("[peak norm] min: {} | max: {}".format(spec.min(), spec.max()))
        return spec[0]


def get_raw_transforms(train, size,
                       wrap_pad_prob=0.5,
...
def get_raw_contrastive_transforms(size,
                                   wrap_pad_prob=0.5,
                                   sample_rate=16000,
                                   min_duration=2,
                                   max_clip_value=0.2,
...
def simple_raw_contrastive_transforms(size,
                                      sample_rate=8000):
    # if train:
    tfs = []
    tfs.append(PeakNormalization(sr=sample_rate))
...
def simple_raw_contrastive_transforms_v2(is_train, size,
                                         sample_rate=8000):
    tfs = []
    if is_train:
        tfs.append(UseWithProb(RandomGain(sr=sample_rate), prob=0.5))
        tfs.append(UseWithProb(AddGaussianNoise(), prob=0.5))
...
def simple_raw_contrastive_transforms_v3(is_train, size,
                                         audio_config,
                                        bg_prob=0.5):
    apply_random_gain = audio_config.get('apply_random_gain', False)
    apply_gaussian_noise = audio_config.get('apply_gaussian_noise', False)
    apply_time_masking = audio_config.get('apply_time_masking', False)
    bg_noise_path = audio_config.get('bg_noise_path', None)
    sample_rate = audio_config.get("sample_rate", 8000)
    min_duration = audio_config.get("min_duration", 10.)
    ...
    if is_train:
        ...
        if apply_gaussian_noise:
            tfs.append(UseWithProb(AddGaussianNoise(), prob=0.5))
        if bg_noise_path:
            random_clip_size = int(audio_config['random_clip_size'] * audio_config['sample_rate'])
            noise_gen = BackgroundNoiseGenerator(bg_noise_path, sr=sample_rate, 
                                                 min_duration=audio_config['random_clip_size'],
                                                 num_samples=random_clip_size,
                                                 waveform_parser=RawAudioParser(
                                                     normalize_waveform=normalize
                                                ))
            bg_tf = AddRandomNoise(noise_gen)
            tfs.append(UseWithProb(bg_tf, prob=bg_prob))

    tfs.append(PeakNormalization(sr=sample_rate))
    if is_train and apply_time_masking:
        tfs.append(TimeMasking(time_perc=0.1, num_masks=3))
    transforms = Compose(tfs)
    return transforms
