...
if __name__ == '__main__':
    ...
    ckpt_path = ckpts[-1]
    print(ckpt_path)
    res_file = os.path.join(args.exp_dir, "results_latest.txt")
    ...
    val_set = RawWaveformDataset("/media/user/nvme/datasets/audioset/meta_8000/eval.csv",
                                 "/media/user/nvme/datasets/audioset/meta_8000/lbl_map.json",
                                 hparams.cfg['audio_config'], mode='multilabel',
                                 transform=val_tfs, is_val=True, delimiter=";"
                                 )

    with open("/media/user/nvme/datasets/audioset/meta_8000/lbl_map.json", "r") as fd:
        lbl_map = json.load(fd)

    inv_map = {v: k for k, v in lbl_map.items()}

    val_loader = DataLoader(val_set, sampler=None, num_workers=2,
    ...
    val_gts = torch.cat(val_gts).detach().cpu().numpy()
